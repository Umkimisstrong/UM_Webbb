...
public class BoardDAO
{
	...
	// 댓글 가져오기
	public ArrayList<BoardDTO> getRes(int num) throws SQLException
	{
		
		ArrayList<BoardDTO> result = new ArrayList<BoardDTO>();
		String sql = "SELECT RECONTENT, IPADDR FROM TBL_RES WHERE NUM = ?"
				   + " ORDER BY RENUM";
		...
		{
			BoardDTO dto = new BoardDTO();
			dto.setRes(rs.getString("RECONTENT"));
			dto.setIpAddr(rs.getString("IPADDR"));
			result.add(dto);
		}
		rs.close();
	...
	// 게시물 작성 → 데이터 입력 메소드
	public int insertRes(BoardDTO dto) throws SQLException
	{
		int result = 0;
		String sql = "INSERT INTO TBL_RES(NUM, RENUM, RECONTENT, IPADDR, CREATED)"
				+ " VALUES(?, RES_SEQUENCE.NEXTVAL, ?, ?, SYSDATE)"				;
		
		PreparedStatement pstmt = conn.prepareStatement(sql);
		
		//dto.setNum(getMaxNum()+1);
		pstmt.setInt(1, dto.getNum());
		pstmt.setString(2, dto.getRes());
	...
	// 게시물 작성 → 데이터 입력 메소드
	public int countRes(int num) throws SQLException
	{
		int result = 0;
		String sql = "SELECT COUNT(*) AS COUNT"
				   + " FROM TBL_RES"
